    ...
    public class DataQueueController
    {
        ...
        public async Task ProcessRecord(PhoneQueue record, Dictionary<int, string> LoadStatusList, Dictionary<int, int> QueueSourceList)
        {
            try
            {
                ...
                else if (record.Command == "removecall")
                {
                    await RemoveFromPresence(record);
                    await SendToPhoneQueueDone(record, "Removed from Presence");
                }
                else
                {
                    bool IsEnabled = LoadStatusList.ContainsValue(record.ServiceID + "," + record.LoadID + ",E");
        ...
        #region [ Is Same Service ]
        public async Task<bool> IsSameServiceAndLoad(PhoneQueue data)
        {
            OutboundQueueLoad dataOut = new OutboundQueueLoad();
            try
            {
                string query = @"SELECT TOP 1 [SERVICEID], [LOADID] FROM [PREP].[PCO_OUTBOUNDQUEUE] WHERE [SOURCEID] = @SOURCEID";

                using IDbConnection connection = new SqlConnection(_dataService.GetConnectionString("Presence"));
                {
                    var dataOutElement = await connection.QueryAsync<OutboundQueueLoad>(query, new { SOURCEID = data.SourceID });
                    dataOut = dataOutElement.FirstOrDefault();
                }
