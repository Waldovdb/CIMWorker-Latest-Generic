                    ...
                    if (!string.IsNullOrEmpty(Header))
                    {
                        if (!string.IsNullOrEmpty(model.HeaderText))
                        {
                            ...
                            else
                            {
                                ...
                                if (!string.IsNullOrEmpty(model.ReplacementHeader))
                                {
                                    lineChanger(model.ReplacementHeader, file.FullName, 1);
                                    await _logService.InfoAsync($"File Header Replaced from {Header} to {model.ReplacementHeader}");
                                }
                                return true;
                            }
                        }
